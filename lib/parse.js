...
class NoteFile {
  ...
  restoreBlock(cap) {
    if (!cap) return false;
    const rebuilt = cap.lines.map(l => {
      const t = parseTaskLine(l);
      return t ? { type: 'task', task: t } : { type: 'raw', text: l };
    });
    const idx = this.doc.findIndex(e => e.type === 'task' && e.task.title === cap.title && e.task.checked);
    if (idx !== -1) { // mutated copy still present — swap it out
      let k = idx + 1;
      while (k < this.doc.length && this.doc[k].type === 'task' && this.doc[k].task.subtask) k++;
      this.doc.splice(idx, k - idx, ...rebuilt);
    } else { // fell off somewhere else — put it back near its origin
      this.doc.splice(Math.min(cap.index, this.doc.length), 0, ...rebuilt);
    }
    this._index();
    return true;
  }
