...
// undo toast — completes from anywhere surface here too
let undoToastT = null;
window.api.onShowUndo(d => {
  const toast = $('undo-toast');
  clearInterval(undoToastT);
  const isDel = d.kind === 'delete';
  let left = d.left;
  toast.innerHTML = `<span class="undo-label">${isDel ? 'Deleted' : 'Completed'}: ${esc(d.label)}</span>
